...
<Dialog
  {open}
  onClose={handleClose}
  title="Render 3D Model to Sprites"
  width="lg"
  bordered
  scrollBody
  {error}
>
    ...
    <div>
      ...
      <div class="flex gap-2">
        <input
          ...
          class="flex-1 px-3 py-2 bg-[var(--color-surface)] rounded-sm border border-[var(--color-surface)] focus:border-[var(--color-neon)] focus:shadow-[var(--glow-neon)] focus:outline-none"
          placeholder="/path/to/model.gltf"
        />
        <button
          onclick={handleBrowse}
          class="px-4 py-2 bg-[var(--color-surface)] hover:bg-[var(--color-surface)]/80 rounded-sm border border-[var(--color-surface)] transition-colors"
        >
          Browse
        </button>
      </div>
          ...
          {#each customAngles as angle, index}
            <div class="flex items-center gap-2">
              <div class="flex-1 grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input
                    ...
                    class="w-full px-2 py-1 bg-[var(--color-surface)] rounded border border-[var(--color-surface)] focus:border-[var(--color-neon)] focus:shadow-[var(--glow-neon)] focus:outline-none"
                  />
                </div>
                <div>
              ...
              <button
                ...
                class="p-1 text-[var(--color-destructive)] hover:text-red-300"
              >
  ...
  {#snippet footer()}
    ...
    <Button onclick={handleRender} disabled={loading || !modelFilePath.trim() || (cameraPreset === 'custom' && customAngles.length === 0)}>
      {#if loading}
        Rendering...
      {:else}
        Render
